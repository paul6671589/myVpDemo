 ...
 * <p>
 * WaveformView 根据给定的音频文件转换成完整的波形
 */
...
public class WaveFormView2 extends View {
    ...
    private Paint mSelectedLinePaint = null;
    private Paint mUnselectedLinePaint = null;
    private Paint mUnselectedBkgndLinePaint = null;
    private Paint mBorderLinePaint = null;
    private Paint mPlaybackLinePaint = null;
    private Paint mTimecodePaint = null;
    ...
    public double pixelsToSeconds(int pixels) {
        ...
        return (mSoundFile.getmNumFramesFloat()
                * 2 * (double) mSamplesPerFrame / (mSampleRate * z));
    }
   ...
   /* public int pixelsToMillisecs(int pixels) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(0, height * 0.5f + line_offset / 2,
                measuredWidth, height * 0.5f + line_offset / 2, centerLine); // 中心线

        paintLine = new Paint();
        paintLine.setColor(Color.rgb(169, 169, 169));
        canvas.drawLine(0, line_offset / 2, measuredWidth,
                line_offset / 2, paintLine); // 最上面的那根线
        canvas.drawLine(0, measuredHeight - line_offset / 2 - 1,
                measuredWidth, measuredHeight - line_offset / 2 - 1, paintLine); // 最下面的那根线
        if (state == 1) {
            mSoundFile = null;
            state = 0;
            return;
        ...
        if (mSoundFile == null) {
            ...
            canvas.drawLine(0, line_offset / 2, measuredWidth,
                    line_offset / 2, paintLine); // 最上面的那根线
            canvas.drawLine(0, measuredHeight - line_offset / 2 - 1,
                    measuredWidth, measuredHeight - line_offset / 2 - 1,
                    paintLine); // 最下面的那根线
            return;
        }
        ...
        for (i = 0; i < maxPos(); i++) {
            ...
            if (i + start == mPlaybackPos && playFinish != 1) {
                canvas.drawCircle(i * getMeasuredWidth() / maxPos(),
                        line_offset / 4, line_offset / 4, circlePaint); // 上圆
                canvas.drawCircle(i * getMeasuredWidth() / maxPos(),
                        getMeasuredHeight() - line_offset / 4,
                        line_offset / 4, circlePaint); // 下圆
                canvas.drawLine(i * getMeasuredWidth() / maxPos(), 0,
                        i * getMeasuredWidth() / maxPos(),
